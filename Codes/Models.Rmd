...
```{r}
# Model 2: Interaction
model3 <- glm(poverty_level_post ~ woman_emp:hh_label+woman_age+woman_edu+num_kids+pre_school +mspouse_age + mspouse_edu+mspouse_emp,
              data = df, 
              family=binomial())

model3_OR <- exp(coef(model3))
model3_CI <- exp(confint(model3))
model3_p.values <- list(summary(model3)$coefficients[,4])
